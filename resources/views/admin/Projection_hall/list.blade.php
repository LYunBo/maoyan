...
<div class="page-container">
	<div class="text-c">
		...
		<input type="text" class="input-text" style="width:250px" placeholder="电影院名称" id="" name="name">
		<button type="submit" class="btn btn-success radius" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜电影院</button>
		</form>
	</div>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		@foreach($data as $row)
			<tr class="text-c">
				...
				<td>{{$row->brand}}</td>
				<td>{{$row->service}}</td>
				<td class="td-manage"><a title="编辑" href="/adminfilmcinema/{{$row-> id}}/edit" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a title="删除" href="javascript:;" class="ml-5 del" onclick="del(this,{{$row -> id}})" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
		@endforeach
		</tbody>
	</table>
	...
	<script>
		function del(thiss,id){
			...
			if (trues) {
				$.get("/adminfilmcinemadel",{"id":id},function(result){
					if (result == "1") {
